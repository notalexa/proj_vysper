...
public class SubscriberPayloadNotificationVisitor implements SubscriberVisitor {
    ...
    private DeliveryFailureStrategy dfs = DeliveryFailureStrategy.IGNORE;
     ...
     */
    public SubscriberPayloadNotificationVisitor(ServerRuntimeContext context,Entity serverJID, XMLElement item) {
    ...
    public void visit(String nodeName, String subscriptionID, Entity subscriber) {
        ...
        if(!context.relay(event, dfs)) {
            if (logger.isTraceEnabled()) {
                logger.trace("Couldn't deliver message to " + subscriber.getFullQualifiedName());
            }
