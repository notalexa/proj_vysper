...
public class BoshHandler {
    ...
    private ServerRuntimeContextProvider contextProvider;
...
//    }
     ...
     */
    public void setServerRuntimeContextProvider(ServerRuntimeContextProvider contextProvider) {
    ...
    public void process(HttpServletRequest httpRequest, Stanza body) {
        ...
        if (body.getAttribute("sid") == null) {
            ...
            if (session == null) {
                LOGGER.warn("Received an invalid 'sid="+body.getAttributeValue("sid")+"'! (sessions="+sessions+")");
                return;
            }
            synchronized (session) {
                ...
                for (;;) {
                    ...
                    session.processDelayedResponses(br);
                }
            }
        }
    }
    ...
    private void createSession(BoshRequest br) throws IOException {
        if (br.getBody().getAttribute("to") != null) {
        	String domain=br.getBody().getAttributeValue("to");
        	ServerRuntimeContext context=contextProvider.resolveDomainContext(domain);
        	if(context!=null) {
		        BoshBackedSessionContext session = new BoshBackedSessionContext(this, context, inactivityChecker);
    ...
    private Stanza getSessionCreationResponse(BoshBackedSessionContext session) {
       ...
       // body.addAttribute("maxpause", Integer.toString(session.getMaxPause()));
        ...
        Stanza features = new ServerResponses().getFeaturesForAuthentication(session.getServerRuntimeContext().getServerFeatures()
                .getAuthenticationMethods(),true);
