...
public class BoshBackedSessionContext extends AbstractSessionContext implements StanzaWriter {
    ...
    void write0(Stanza response) {
        if (requestsWindow.isEmpty() || requestsWindow.firstKey() > highestReadRid) {
            ...
            write0(requestsWindow.get(requestsWindow.firstKey()),response);
        }
    }
    
    void write0(BoshRequest req,Stanza response) {
    	if(req.getRid()!=requestsWindow.firstKey()) {
    		req=requestsWindow.get(requestsWindow.firstKey());
    	}
    	requestsWindow.remove(req.getRid());
    ...
    public void processDelayedResponses(BoshRequest req) {
        ...
        if (requestsWindow.size() > hold) {
            write0(req,boshHandler.getEmptyResponse());
        }    	
    }
