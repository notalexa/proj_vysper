...
public class RosterIQHandler extends DefaultIQHandler {
    ...
    private Stanza rosterItemRemove(IQStanza stanza, SessionContext sessionContext, RosterManager rosterManager,
        ...
        if (unsubscribedStanza != null) {
            if(!sessionContext.getServerRuntimeContext().relay(unsubscribedStanza)) {
                logger.warn("failure sending unsubscribed on roster remove");
            }
        }
        if (unsubscribeStanza != null) {
            if(!sessionContext.getServerRuntimeContext().relay(unsubscribeStanza)) {
                logger.warn("failure sending unsubscribe on roster remove");
            }
        }
